...
API_KEY = 'AIzaSyBZaDgNxtXC1DEwODP1f9leL4TFszUMMmA'
...
# Streamlit app
def main():
    st.title("Imagenes a HTMl Conversor usando Gemini A")
    st.subheader('SoCiE- ISII')
      # Display SoCiE logo (if available)
    image_path = "./img/socie_logo.png"  # Replace with actual path
    with st.container():
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("CODIGO UI"):
                st.write("🧑‍💻 ANALIZANDO LA UI...")
                prompt = "Describe this UI in accurate details. When you reference a UI element put its name and bounding box in the format: [object name (y_min, x_min, y_max, x_max)]. Also Describe the color of the elements."
                description = send_message_to_model(prompt, temp_image_path)
                st.write(description)

                # Refine the description
                st.write("🔍 REFINADO Y COMPARANDO CON LA DESCRIPCION VISUAL...")
                ...
                st.success("ARCHIVO HTML'index.html' A SIDO CREADO.")
